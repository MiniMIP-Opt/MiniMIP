...
NodeIndex MipTree::AddNodeByBranchingFromParent(
  ...
  VLOG(4) << "calling AddNodeByBranchingFromParent().";
...
void MipTree::CloseNodeAndReclaimNodesUpToRootIfPossible(NodeIndex n) {
  VLOG(4) << "calling CloseNodeAndReclaimNodesUpToRootIfPossible().";
...
void MipTree::SetLpRelaxationDataInNode(NodeIndex n,
  ...
  VLOG(4) << "calling SetLpRelaxationDataInNode().";
  CHECK_GE(n, kRootNode);
  CHECK_LT(n.value(), nodes_.size());
  CHECK(NodeIsActive(n));
...
void MipTree::SetImpliedVariableBoundsInNode(
  ...
  VLOG(4) << "calling SetImpliedVariableBoundsInNode().";
  CHECK_GE(n, kRootNode);
  CHECK_LT(n.value(), nodes_.size());
  CHECK(NodeIsActive(n));
...
DenseRow MipTree::RetrieveLowerBounds(NodeIndex n,
  ...
  VLOG(4) << "calling RetrieveLowerBounds().";
  CHECK_GE(n, kRootNode);
  CHECK_LT(n.value(), nodes_.size());
  CHECK(NodeIsActive(n));
...
DenseRow MipTree::RetrieveUpperBounds(NodeIndex n,
  ...
  VLOG(4) << "calling RetrieveUpperBounds().";
  CHECK_GE(n, kRootNode);
  CHECK_LT(n.value(), nodes_.size());
  CHECK(NodeIsActive(n));
...
void MipTree::GenerateDotFile(const NodeIndex& current_node,
  ...
  if (!file.is_open()) {
    std::cerr << "Failed to open file for writing: " << filepath + ".dot"
              << "\n";
    return;
  }

  file << "digraph G {\n";
  for (NodeIndex n = kRootNode; n < nodes_.size(); ++n) {
    if (NodeIsActive(n)) {
      const NodeData& node_data = nodes_[n];
      std::string color = "black";  // Default color for unprocessed nodes
      std::string style = "filled";
      std::string fillcolor = "white";  // Default fill color
      if (n == current_node) {
        color = "black";
        fillcolor =
            "lightgrey";  // Example: processed nodes are filled with light grey
      }
      if (node_data.parent != kInvalidNode) {
        file << "  " << node_data.parent.value() << " -> " << n.value()
             << ";\n";
      }
      file << "  " << n.value() << " [label=\" Node " << n.value() << "\\n"
           << "LPobj: " << node_data.lp_objective_value << "\\n"
           << "LB: " << node_data.implied_lower_bounds.size() << "\\n"
           << "UB: " << node_data.implied_upper_bounds.size() << "\\n"
           << "B_var: " << node_data.branch_variable << "\\n"
  ...
  file.flush();
  file.close();
  std::cout << "DOT file generated: " << filepath << "\n";
  ...
  if (result != 0) {
    // The command failed; you can handle the error as appropriate for your
    // application.
