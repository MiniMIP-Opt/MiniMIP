...
void LPSoplexInterface::SetFeasibilityTolerance(const soplex::Real d) {
  DCHECK(spx_->setRealParam(soplex::SoPlex::FEASTOL, d));
}
...
void LPSoplexInterface::SetOptimalityTolerance(const soplex::Real d) {
  DCHECK(spx_->setRealParam(soplex::SoPlex::OPTTOL, d));
}
...
void LPSoplexInterface::TrySolve(bool print_warning = true) {
  ...
  try {
    ...
    // anymore.
    DCHECK(spx_->status() != soplex::SPxSolver::OPTIMAL);
  }
  DCHECK(spx_->intParam(soplex::SoPlex::ITERLIMIT) < 0 ||
  ...
  if (time_spent > 0) {
    ...
    // set new time limit
    DCHECK_LE(time_limit, 0);

    DCHECK(spx_->setRealParam(soplex::SoPlex::TIMELIMIT, time_limit));
  }
}
...
soplex::SPxSolver::Status LPSoplexInterface::doSolve(
  ...
  DCHECK(CheckConsistentBounds());
  DCHECK(CheckConsistentSides());
...
absl::Status LPSoplexInterface::SoPlexSolve() {
  ...
  DCHECK(PreStrongBranchingBasisFreed());
  ...
  DCHECK(!GetFromScratch() || spx_->status() == soplex::SPxSolver::NO_PROBLEM);
...
absl::Status LPSoplexInterface::LoadSparseColumnLP(
  ...
  for (size_t i = 0; i < cols.size(); i++) {
    for (size_t j = 0; j < cols[i].NumNonZeros(); j++) {
      DCHECK(cols[i].ValuesData()[j] != 0.0);
    }
  }
#endif
...
absl::Status LPSoplexInterface::AddColumn(
  ...
  for (size_t j = 0; j < col.NumNonZeros(); ++j) {
    DCHECK_LE(0, col.IndicesData()[j]);
    DCHECK_LT(col.IndicesData()[j], num_rows);
    DCHECK(col.ValuesData()[j] != 0.0);
  }
#endif

  try {
    soplex::DSVector col_vector;
    col_vector.add(col.NumNonZeros(), col.IndicesData(), col.ValuesData());
    spx_->addColReal(
...
absl::Status LPSoplexInterface::AddColumns(
  ...
  if (!cols.empty()) {
    ...
    for (size_t i = 0; i < cols.size(); i++) {
      const int* indices   = cols[i].IndicesData();
      const double* values = cols[i].ValuesData();
      for (size_t j = 0; j < cols[i].NumNonZeros(); ++j) {
        DCHECK_LE(0, indices[j]);
        DCHECK_LT(indices[j], num_rows);
        DCHECK(values[j] != 0.0);
      }
    }
  }
  ...
  try {
    ...
    for (size_t i = 0; i < cols.size(); ++i) {
      ...
      if (!cols[i].empty()) {
        start = (i == 0 ? 0 : last);
        last  = start + cols[i].NumNonZeros();
        col_Vector.add(last - start, indices, values);
      }
...
absl::Status LPSoplexInterface::AddRow(
  ...
  for (size_t j = 0; j < row.NumNonZeros(); ++j) {
    DCHECK(row.ValuesData()[j] != 0.0);
    DCHECK_LE(0, row.IndicesData()[j]);
    DCHECK_LT(row.IndicesData()[j], num_cols);
  }
#endif

  soplex::DSVector row_vector;
  row_vector.add(row.NumNonZeros(), row.IndicesData(), row.ValuesData());
...
absl::Status LPSoplexInterface::AddRows(
  ...
  for (size_t i = 0; i < rows.size(); i++) {
    if (!rows[i].empty()) {
      ...
      // mistake
      int num_cols = spx_->numColsReal();
      for (size_t j = 0; j < rows[i].NumNonZeros(); ++j) {
        DCHECK(rows[i].ValuesData()[j] != 0.0);
        DCHECK_LE(0, rows[i].IndicesData()[j]);
        DCHECK_LT(rows[i].IndicesData()[j], num_cols);
      }
    }
  }
#endif
  ...
  try {
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
      row_Vector.clear();
      if (!rows[i].empty()) {
        auto start = (i == 0 ? 0 : last);
        last       = start + rows[i].NumNonZeros();
        row_Vector.add(last - start, rows[i].IndicesData(),
                       rows[i].ValuesData());
      }
...
absl::Status LPSoplexInterface::DeleteRows(
  ...
  DCHECK(last_row < spx_->numRowsReal());
...
absl::Status LPSoplexInterface::SetObjectiveCoefficients(
  ...
  try {
    for (i = 0; i < indices.size(); ++i) {
      DCHECK(0 <= indices[i]);
      DCHECK(indices[i] < spx_->numColsReal());
...
SparseVector LPSoplexInterface::GetSparseColumnCoefficients(int col) const {
  ...
  if (spx_->boolParam(soplex::SoPlex::PERSISTENTSCALING)) {
    ...
    for (int j = 0; j < cvec.size(); ++j) {
      sparse_column.InsertSorted(cvec.index(j), cvec.value(j));
    }
  } else {
...
SparseVector LPSoplexInterface::GetSparseRowCoefficients(int row) const {
  ...
  if (spx_->boolParam(soplex::SoPlex::PERSISTENTSCALING)) {
    ...
    for (int j = 0; j < rvec.size(); ++j) {
      sparse_row.InsertSorted(rvec.index(j), rvec.value(j));
    }
  } else {
...
bool LPSoplexInterface::IsPrimalUnbounded() const {
  ...
  DCHECK(spx_->status() != soplex::SPxSolver::UNBOUNDED ||
...
bool LPSoplexInterface::IsOptimal() const {
  ...
  DCHECK((spx_->basisStatus() == soplex::SPxBasis::OPTIMAL) ==
...
absl::StatusOr<SparseVector> LPSoplexInterface::GetSparseRowOfBInverted(
  ...
  DCHECK(PreStrongBranchingBasisFreed());
  DCHECK(row_number >= 0);
  DCHECK(row_number < spx_->numRowsReal());
  ...
  auto sparse_row = SparseVector();
  for (int i = 0; i < num_indices; ++i) {
    sparse_row.InsertSorted(indices[i], dense_row[indices[i]]);
  }
...
absl::StatusOr<SparseVector> LPSoplexInterface::GetSparseColumnOfBInverted(
  ...
  for (int i = 0; i < num_indices; ++i) {
    sparse_column.InsertSorted(indices[i], dense_column[indices[i]]);
  }
...
absl::StatusOr<SparseVector> LPSoplexInterface::GetSparseRowOfBInvertedTimesA(
  ...
  for (int col = 0; col < num_cols; ++col) {
    ...
    if (REALABS(val) > EPS) {
      sparse_row.InsertSorted(col, val);
    }
  }
  return sparse_row;
}
...
LPSoplexInterface::GetSparseColumnOfBInvertedTimesA(int col_number) const {
  ...
  DCHECK(col_number >= 0);
  DCHECK(col_number < spx_->numColsReal());
  ...
  for (int row = 0; row < num_rows; ++row) {
    if (REALABS(binv_vec[row]) > EPS) {
      sparse_col.InsertSorted(row, binv_vec[row]);
    }
  }
  return sparse_col;
}
  ...
  switch (type) {
    ...
    case LPParameter::kScaling:
      ...
      if (scale_param == soplex::SoPlex::SCALER_OFF) {
        ...
        DCHECK(scale_param == soplex::SoPlex::SCALER_LEASTSQ);
        param_val = 2;
      }
      break;
...
absl::Status LPSoplexInterface::SetIntegerParameter(
  ...
  switch (type) {
    case LPParameter::kFromScratch:
      DCHECK(param_val == true || param_val == false);
    ...
    case LPParameter::kPricing:
      ...
      break;
    case LPParameter::kScaling:
      DCHECK(param_val >= 0 && param_val <= 2);
      if (param_val == 0)
      ...
      break;
    case LPParameter::kTiming:
      DCHECK(param_val >= 0 && param_val < 3);
...
absl::Status LPSoplexInterface::ReadLP(const char* file_name  // file name
  ...
  try {
    DCHECK(spx_->intParam(soplex::SoPlex::READMODE) ==
